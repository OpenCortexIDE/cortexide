	...
	(async () => {
		try {
			if (packageJsonConfig.dependencies) {
				const webpackRootConfig = await loadWebpackConfigModule(path.join(extensionPath, webpackConfigFileName));
			...
			const fileNames = await vsce.listFiles({ cwd: extensionPath, packageManager: vsce.PackageManager.None, packagedDependencies });
			...
			const webpackStreamGroups = await Promise.all(webpackConfigLocations.map(async webpackConfigPath => {
				...
				const exportedConfig = await loadWebpackConfigModule(webpackConfigPath);
				const configs = Array.isArray(exportedConfig) ? exportedConfig : [exportedConfig];
			...
			const webpackStreams = (<Stream[]>[]).concat(...webpackStreamGroups);
...
async function loadWebpackConfigModule(webpackConfigPath: string): Promise<any> {
	let resolvedPath = webpackConfigPath;
	if (resolvedPath.endsWith('.js')) {
		const mjsPath = resolvedPath.replace(/\.js$/, '.mjs');
		try {
			const srcStat = fs.statSync(resolvedPath);
			const destStat = fs.existsSync(mjsPath) ? fs.statSync(mjsPath) : undefined;
			if (!destStat || srcStat.mtimeMs > destStat.mtimeMs) {
				fs.copyFileSync(resolvedPath, mjsPath);
			}
		} catch (error) {
			// ignore copy errors, fallback to original path
		}
		resolvedPath = mjsPath;
	}

	const module = await import(pathToFileURL(path.resolve(resolvedPath)).href);
	return (module && Object.prototype.hasOwnProperty.call(module, 'default')) ? module.default : module;
}
