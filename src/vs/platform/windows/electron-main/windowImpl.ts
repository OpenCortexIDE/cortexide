...
class DockBadgeManager {
	...
	acquireBadge(window: IBaseWindow): IDisposable {
		...
		electron.app.setBadgeCount(
			isLinux ? 1 /* only numbers supported */ : undefined /* generic dot */,
		);
...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	private readonly _onDidChangeAlwaysOnTop = this._register(
		new Emitter<boolean>(),
	);
	...
	protected setWin(
		...
		this._register(
			Event.fromNodeEventEmitter(
				...
				if (
					isWindows &&
					this.environmentMainService.enableRDPDisplayTracking &&
					this._win
				) {
					const [x, y] = this._win.getPosition();
					const [width, height] = this._win.getSize();

					this.maximizedWindowState = {
					...
					this.logService.debug(
						`Saved maximized window ${this.id} display state:`,
						this.maximizedWindowState,
					);
				}
		...
		this._register(
			Event.fromNodeEventEmitter(
				...
				(_, alwaysOnTop) => alwaysOnTop,
			)((alwaysOnTop) => this._onDidChangeAlwaysOnTop.fire(alwaysOnTop)),
		);
		...
		if ((isWindows || isLinux) && useCustomTitleStyle) {
			this._register(
				Event.fromNodeEventEmitter(
					win,
					"system-context-menu",
					(event: Electron.Event, point: Electron.Point) => ({ event, point }),
				)((e) => {
		...
		if (isWindows && this.environmentMainService.enableRDPDisplayTracking) {
			...
			this._register(
				Event.fromNodeEventEmitter(
					screen,
					"display-added",
					(event: Electron.Event, display: Display) => ({ event, display }),
				)((e) => {
	...
	private onDisplayAdded(display: Display): void {
		...
		if (
			state &&
			this._win &&
			WindowStateValidator.validateWindowStateOnDisplay(state, display)
		) {
			this.logService.debug(
				`Setting maximized window ${this.id} bounds to match newly added display`,
				state,
			);
	...
	private readonly notifyFocusDisposable = this._register(
	...
	private doSetNativeFullScreen(
		...
		if (isMacintosh) {
			...
			const joinNativeFullScreenTransition =
				(this.joinNativeFullScreenTransition = new DeferredPromise<boolean>());
			(async () => {
				...
				if (
					this.joinNativeFullScreenTransition !== joinNativeFullScreenTransition
				) {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			this.configObjectUrl = this._register(
				protocolMainService.createIPCObjectUrl<INativeWindowConfiguration>(),
			);
		...
		let sampleInterval = parseInt(
			this.environmentMainService.args["unresponsive-sample-interval"] ||
				"1000",
		);
	...
	private registerListeners(): void {
				...
				Event.fromNodeEventEmitter(
					...
					if (!readyToShowFired && this._win && !this._win.isDestroyed()) {
						...
						if (bounds && (bounds.width === 0 || bounds.height === 0)) {
							this.logService.warn(
								"window#ready-to-show: invalid bounds, resetting",
							);
		...
		this._register(
			this.workspacesManagementMainService.onDidDeleteUntitledWorkspace((e) =>
				this.onDidDeleteUntitledWorkspace(e),
			),
		);
						...
						await this.dialogMainService.showMessageBox(
							{
								...
								buttons: [
									localize(
										{ key: "reopen", comment: ["&& denotes a mnemonic"] },
										"&&Reopen",
									),
									localize(
										{ key: "close", comment: ["&& denotes a mnemonic"] },
										"&&Close",
									),
									localize(
										{ key: "wait", comment: ["&& denotes a mnemonic"] },
										"&&Keep Waiting",
									),
								],
	...
	private onConfigurationUpdated(e?: IConfigurationChangeEvent): void {
		...
		if (
			...
			const newNoProxy =
				(this.configurationService.getValue<string[]>("http.noProxy") || [])
					.map((item) => item.trim())
					.join(",") ||
	...
	load(
		...
		const workbenchUri = FileAccess.asBrowserUri(
			`vs/code/electron-browser/workbench/${workbenchFileName}`,
		);
		const workbenchUrl = workbenchUri.toString(true);

		// Enhanced diagnostics for workbench loading
		this.logService.trace(
			`window#load: Loading workbench from: ${workbenchUrl}`,
		);
		this.logService.trace(
			`window#load: workbenchFileName: ${workbenchFileName}`,
		);
		this.logService.trace(
			`window#load: isBuilt: ${this.environmentMainService.isBuilt}`,
		);
		this.logService.trace(
			`window#load: appRoot: ${this.environmentMainService.appRoot}`,
		);

		// Verify workbench file exists (critical for macOS blank screen issue)
		const workbenchFileUri = FileAccess.asFileUri(
			`vs/code/electron-browser/workbench/${workbenchFileName}`,
		);
		const workbenchFilePath = workbenchFileUri.fsPath;
		if (!existsSync(workbenchFilePath)) {
			this.logService.error(
				`window#load: CRITICAL - workbench.html file does not exist at: ${workbenchFilePath}`,
			);
			this.logService.error(
				`window#load: This will cause a blank screen. Expected file: ${workbenchFileName}`,
			);
			this.logService.error(
				`window#load: appRoot: ${this.environmentMainService.appRoot}`,
			);
			...
			this.logService.trace(
				`window#load: ✓ workbench.html file exists at: ${workbenchFilePath}`,
			);
		}
		...
		if (isMacintosh && this._win) {
			...
			const ensureWindowVisible = () => {
				if (this._win && !this._win.isDestroyed()) {
					...
					if (!wasVisible) {
						this.logService.trace(
							"window#load: forcing window to show on macOS",
						);
						this._win.showInactive();
						windowShown = true;
					} else {
					...
					if (bounds && (bounds.width === 0 || bounds.height === 0)) {
						this.logService.warn(
							"window#load: invalid window bounds detected, resetting to default size",
						);
						this._win.setSize(1024, 768);
						this._win.center();
					}
			...
			this._win.webContents.once(
				"did-fail-load",
				(event, errorCode, errorDescription, validatedURL) => {
					this.logService.error(
						`window#load: Failed to load workbench on macOS - Code: ${errorCode}, Description: ${errorDescription}, URL: ${validatedURL}`,
					);
					this.logService.error(
						"window#load: This may indicate workbench.html is missing or the file path is incorrect",
					);
					this.logService.error(
						`window#load: Expected file: ${workbenchInfo.fileName}`,
					);
					this.logService.error(
						`window#load: Constructed URI: ${workbenchInfo.url}`,
					);
			...
			this._win.webContents.once("render-process-gone", (event, details) => {
				this.logService.error(
					`window#load: Renderer process crashed on macOS - Reason: ${details.reason}, Exit Code: ${details.exitCode}`,
				);
				if (details.reason === "crashed" || details.reason === "killed") {
					this.logService.error(
						"window#load: This may indicate a GPU/rendering issue. Try launching with --disable-gpu flag.",
					);
				}
			...
			this._win.webContents.once("dom-ready", () => {
				this.logService.trace(
					"window#load: dom-ready event, ensuring window visibility",
				);
				ensureWindowVisible();
			});
			...
			setTimeout(() => {
				if (this._win && !this._win.isDestroyed()) {
					if (!this._win.isVisible()) {
						this.logService.warn(
							"window#load: window still not visible after delay, forcing show",
						);
						ensureWindowVisible();
					}
					...
					if (!windowShown) {
						this.logService.error(
							"window#load: CRITICAL - Window was never shown on macOS. This indicates a serious issue.",
						);
						this.logService.error("window#load: Possible causes:");
						this.logService.error(
							"window#load: 1. workbench.html file is missing from app bundle",
						);
						this.logService.error("window#load: 2. File path is incorrect");
						this.logService.error(
							"window#load: 3. GPU/rendering issue (try --disable-gpu)",
						);
						this.logService.error(
							"window#load: 4. Content Security Policy blocking rendering",
						);
					}
				}
			}, 500);
			...
			this._win.webContents.once("did-frame-finish-load", () => {
				this.logService.trace(
					"window#load: Frame finished loading, verifying window state",
				);
				ensureWindowVisible();
			});
		}
	...
	private async startCollectingJScallStacks(): Promise<void> {
		if (!this.jsCallStackCollector.isTriggered()) {
			const stack =
				await this._win?.webContents.mainFrame.collectJavaScriptCallStack();
	...
	private stopCollectingJScallStacks(): void {
		...
		if (this.jsCallStackMap.size) {
			...
			const sortedEntries = Array.from(this.jsCallStackMap.entries()).sort(
				(a, b) => b[1] - a[1],
			);
			...
			for (const [stack, count] of sortedEntries) {
				...
				if (
					Math.round((count * 100) / this.jsCallStackEffectiveSampleCount) > 20
				) {
					const fakeError = new UnresponsiveError(
						stack,
						this.id,
						this._win?.webContents.getOSProcessId(),
					);
